/*
	...
	Filename:      lzuf62.c  (decompressor:  lzufd62.c)
	...
	Version 2:
		(2/27/2022) Optional bit size for sliding window implemented, BITS = 12..21, default = 17.
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
dpos_t dpos;
unsigned char *win_buf;     /* the "sliding" window buffer. Max = 21 bits or 2MB */
unsigned char *pattern;     /* the "look-ahead" buffer. Max = 20 bits 1MB */
...
void usage( void )
{
	fprintf(stderr, "\n Usage: lzuf62 [-N] infile outfile\n\n where N = nbits size (N = 12..21) of window buffer, default=17.");
	copyright();
	exit (0);
}
...
int main( int argc, char *argv[] )
{
	...
	if ( argc == 4 ) {
		if ( argv[1][0] == '-' ) {
			...
			else if ( num_POS_BITS < 12 ) num_POS_BITS = NUM_POS_BITS;
	...
	fstamp.algorithm[3] = num_POS_BITS;
